...
var Room = {
	...
	// Returns true if the given item is the highest-tier owned item of its group.
	isEquippableBestInGroup: function(thing) {
		for(var g = 0; g < Room.EquippableGroups.length; g++) {
			var group = Room.EquippableGroups[g];
			var idx = group.indexOf(thing);
			if(idx === -1) { continue; }
			// A higher-tier member of this group is owned, so this item should be hidden.
			for(var h = idx + 1; h < group.length; h++) {
				if($SM.get('stores["'+group[h]+'"]', true) > 0) {
					return false;
				}
			}
			return true;
	...
	unequip: function(thing) {
		if(Room.isEquippable(thing)) {
			// Unequipping one item of a group unequips the whole group (the slot).
			for(var g = 0; g < Room.EquippableGroups.length; g++) {
				var group = Room.EquippableGroups[g];
				if(group.indexOf(thing) !== -1) {
					for(var i = 0; i < group.length; i++) {
						$SM.set('game.equipped["'+group[i]+'"]', false);
					}
					return;
				}
			}
	...
	migrateEquip: function() {
		// Once an item is owned, it counts as equipped until the player manually unequips it.
		for(var i = 0; i < Room.EquippableItems.length; i++) {
			var thing = Room.EquippableItems[i];
	...
	init: function(options) {
		...
		// Ensure previously-owned carry items count as equipped
		Room.migrateEquip();
		...
		light.prepend(Pixel.icon('fx_fire', {pixel: 2}));
		
		// Create the stoke button
		var stoke = new Button.Button({
			id: 'stokeButton',
			text: _("stoke fire"),
		...
		stoke.prepend(Pixel.icon('fx_fire', {pixel: 2}));
	...
	updateIncomeView: function() {
		...
		// 在数值后带括号显示该资源的净增减（来自所有收入源）
		var base = Math.floor($SM.get('stores["'+storeName+'"]') || 0);
		var valText = base;
		var netVal = Math.round(net * 10) / 10;  // 保留一位小数，避免出现 (+0)
		if(netVal !== 0) {
			valText = base + ' (' + (netVal > 0 ? '+' : '') + netVal + ')';
		}
		$('div.row_val', el).text(valText);
		});
	},
	...
	build: function(buildBtn) {
		...
		case 'tool':
			$SM.add('stores["'+thing+'"]', 1);
			// Carry items auto-equip when first bought
	...
	updateBuildButtons: function() {
		var attachSpriteIcon = function(btn, key) {
			var spr = Pixel.buildingSprite(key) || Pixel.resourceSprite(key);
			if(spr && spr !== 'res_generic') {
				btn.prepend(Pixel.icon(spr, {pixel: 2}));
			}
		};
		...
		for(var k in Room.Craftables) {
			...
			if(craftable.button == null) {
				if(Room.craftUnlocked(k)) {
					...
					attachSpriteIcon(craftable.button, k);
				}
			} else {
				// refresh the tooltip
				...
				var cost = craftable.cost();
				for(var k in cost) {
					var ck = $("<div>").addClass('row_key').text(_(k));
					ck.prepend(Pixel.icon(Pixel.resourceSprite(k), {pixel: 2}));
					ck.appendTo(costTooltip);
		...
		for(var k in Room.TradeGoods) {
			...
			if(good.button == null) {
				if(Room.buyUnlocked(k)) {
					...
					attachSpriteIcon(good.button, k);
				}
			} else {
				// refresh the tooltip
				...
				var cost = good.cost();
				for(var k in cost) {
					var gk = $("<div>").addClass('row_key').text(_(k));
					gk.prepend(Pixel.icon(Pixel.resourceSprite(k), {pixel: 2}));
					gk.appendTo(costTooltip);
					$("<div>").addClass('row_val').text(cost[k]).appendTo(costTooltip);
				}
