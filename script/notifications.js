...
var Notifications = {
	...
	printMessage: function(t) {
		var icon = (typeof Pixel !== 'undefined' && Pixel.notificationIcon) ? Pixel.notificationIcon(t) : null;
		var text = $('<div>').addClass('notification').css('opacity', '0');
		// 根据消息内容匹配一个像素图标，让通知更好识别
		if(icon) {
			text.append(Pixel.icon(icon, {pixel: 2, className: 'notification-icon'}));
		}
		text.append($('<span>').addClass('notification-text').text(t)).prependTo('div#notifications');
