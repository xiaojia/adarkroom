...
var Outside = {
	...
	makeWorkerRow: function(key, num) {
		...
		for(var s in income.stores) {
			...
			var tooltipKey = $('<div>').addClass('row_key').text(_(s));
			tooltipKey.prepend(Pixel.icon(Pixel.resourceSprite(s), {pixel: 2}));
			tooltipKey.appendTo(r);
			$('<div>').addClass('row_val').text(Engine.getIncomeMsg(income.stores[s] * num, income.delay)).appendTo(r);
			r.appendTo(tooltip);
		}
	...
	updateVillageRow: function(name, num, village) {
		...
		if(row.length === 0 && num > 0) {
			...
			var keyDiv = $('<div>').addClass('row_key').text(_(name));
			var sprite = Pixel.buildingSprite(name);
			if(sprite) {
				keyDiv.prepend(Pixel.icon(sprite, {pixel: 2}));
			}
			keyDiv.appendTo(row);
