var World = {
			...
			for(var i = 0; i <= World.RADIUS * 2; i++) {
				...
				if(World.curPos[0] == i && World.curPos[1] == j) {
					...
					if(landmark && Pixel.svg(landmark)) {
						...
						var cls = Pixel.tileClass(base) || 'px-unknown';
						tileClass += ' ' + cls;
					}
				} else {
					tileClass += ' px-unknown';
				}
				html += '<span class="' + tileClass + '">' + inner + '</span>';
			}
			html += '</div>';
		}
		map.html(html);
	...
	getMaxWater: function() {
		if(Room.isEquipped('water tank')) {
			return World.BASE_WATER + 50;
		} else if(Room.isEquipped('cask')) {
			return World.BASE_WATER + 20;
		} else if(Room.isEquipped('waterskin')) {
	...
	onArrival: function() {
		...
		World.updateSupplies();
		$('#bagspace-world').outerWidth($('#map').outerWidth());
	},
