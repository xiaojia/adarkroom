var Path = {
	...
	updateOutfitting: function() {
		...
		if($SM.get('stores["s armour"]', true) > 0 && Room.isEquipped('s armour'))
			...
			armour = _("iron");
		else if($SM.get('stores["l armour"]', true) > 0 && Room.isEquipped('l armour'))
			armour = _("leather");
		var aRow = $('#armourRow');
		if(aRow.length === 0) {
		...
		// Add the equip/unequip toggles for carry items
		var anyEquip = false;
		for(var e = 0; e < Room.EquippableItems.length; e++) {
			if($SM.get('stores["'+Room.EquippableItems[e]+'"]', true) > 0) {
				anyEquip = true;
				break;
			}
		}
			...
			$('<div>').addClass('equipHeader').text(_('equipment')).appendTo(equipList);
				for(var e = 0; e < Room.EquippableItems.length; e++) {
					...
					if(spr && spr !== 'res_generic') {
						keyDiv.prepend(Pixel.icon(spr, {pixel: 2}));
					}
					eRow.append(keyDiv);
					var eVal = $('<div>').addClass('row_val').appendTo(eRow);
					var equipped = Room.isEquipped(ek);
					var statusSpan = $('<span>').appendTo(eVal);
					statusSpan.append(Pixel.icon(equipped ? 'icon_check' : 'icon_cross', {pixel: 2}));
					statusSpan.attr('title', equipped ? _('equipped') : _('unequipped'));
					$('<div>').addClass('equipBtn').text(equipped ? _('unequip') : _('equip'))
						.data('thing', ek)
						.click(function() { Path.toggleEquip($(this).data('thing')); })
						.appendTo(eVal);
					$('<div>').addClass('clear').appendTo(eRow);
					eRow.appendTo(equipList);
				}
			}
	...
	toggleEquip: function(thing) {
		if(Room.isEquipped(thing)) {
			Room.unequip(thing);
			Notifications.notify(Room, _('{0} {1}', _(thing), _('unequipped')));
		} else {
			Room.equip(thing);
			Notifications.notify(Room, _('{0} {1}', _(thing), _('equipped')));
		}
		Path.updateOutfitting();
	},
